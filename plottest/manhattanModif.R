...
manhattan2 <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
                      col=c("gray10", "gray60"), chrlabs=NULL,
                      suggestiveline=-log10(1e-5), genomewideline=-log10(5e-8), 
                      highlight=NULL, logp=TRUE, annotatePval = NULL, annotateTop = TRUE, annotateSnp=NULL,genes=NULL,...) {
   ...
   d$index = rep.int(seq_along(unique(d$CHR)), times = tapply(d$SNP,d$CHR,length))  # replcace the for loop of line 92-96 to improve efficiency
    ...
    def_args <- list(xaxt='n', bty='n', xaxs='i', yaxs='i', las=1, pch=20,
                     xlim=c(xmin,xmax), ylim=c(0,ceiling(max(d$logp)*1.1)),
    ...
    if (!is.null(highlight)) {
        if(is.list(highlight)){
          colvec=names(highlight)
          for(col in colvec){
            d.highlight=d[which(d$SNP %in% highlight[[col]]), ]
            with(d.highlight, points(pos, logp, col=col, pch=20, ...)) 
          }
        }else {
    ...
    if (! is.null(annotateSnp)){
         SnpHit = subset(d, SNP %in% annotateSnp)
          if (logp) {
              with(subset(d, SNP %in% annotateSnp ), 
                   textxy(pos, -log10(P), offset = 0.625, labs = SnpHit$SNP, cex = 0.7), ...)
          } else
              with(subset(d, SNP %in% annotateSnp), 
                   textxy(pos, P, offset = 0.625, labs = SnpHit$SNP, cex = 0.45), ...)
    } 
    if(!is.null(genes)){
         SnpGenes = subset(d, SNP %in% genes$SNP)
         SnpGenes<-merge(SnpGenes,genes, by="SNP")
         SnpGenes$PosGene<-ceiling(max(d$logp)*1.05)
         if (logp) {
                   print(SnpGenes$pos)
                   with(SnpGenes,textxy(pos, -log10(P)+5, offset = 0, labs = Genes, cex = 0.8, col="blue"), ...)
          } else
              with(subset(d, SNP %in% annotateSnp),
                   textxy(pos, P, offset = 0.625, labs = SnpHit$SNP, cex = 0.45), ...)
    }
