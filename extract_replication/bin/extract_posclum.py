...
def WriteGWASPlk(file_gwas,infopos, fileoutgwas, args, infors, ResLD=None) :
   ...
   cmtline=0
   writegwas=open(fileoutgwas,'w')
   writegwas.write("SNP\tP\n")
     ...
     if (spl[ChroPosGwas] in listchro) :
       ...
       if ResLD :
         ...
         if CheckPosInLPos(infopos[chro], float(bp)) or CheckPosInLPos(ResLD[chro], float(bp)): 
           ...
           writegwas.write(rs+"\t"+Pv+"\n")
           cmtline+=1
       elif CheckPosInLPos(infopos[spl[ChroPosGwas]], float(bp)) :
         if chro not in infogwas :
            infogwas[chro]={}
         infogwas[chro][bp]=[rs,Pv]
         writegwas.write(spl[RsPosGwas]+"\t"+Pv+"\n")
         cmtline+=1
   readgwas.close()
   writegwas.close()
...
def GetResByPos(infopos,data_clump, window, out) :
   ...
   for chro in infopos.keys() :
     ...
     for info in infopos[chro] :
      ...
      if len(lmin)>0 and min(lmin)<window*1000 :
        minlmin=min(lmin)
...
def GetResByRs(infopos,data_clump, window, infors,out):
  cmtpos=0
...
clean=False
### 
(infopos,infors)  = read_list_info(args.list_info, ChroHeadInf,BpHeadInf, window*1000)

### format for plink
fileoutgwas=args.out+".formarplk"
#if os.path.isfile(fileoutgwas)==False or renew:
if args.file_block_ld :
   ResLd=GetLDInfo(infopos, args.file_block_ld)    
else :
   ResLd=None
infogwas=WriteGWASPlk(args.file_gwas, infopos,fileoutgwas, args, infors, ResLd) 
...
if os.path.isfile(fileoutpl)==False or renew:
   print("---- plink launch ----")
   os.system(args.binplinks+" -bfile "+args.bfile+" --clump "+fileoutgwas+" --clump-p1 "+ args.minpval+" --clump-p2 1 "+" --clump-kb "+str(window)+"  --clump-r2 "+args.r2+" --out "+fileoutpltmp+" "+keep+" --threads "+args.cpus)
   print("---- end plink ----")
else :
...
GetResByPos(infopos,data_clump, window, args.out)
GetResByRs(infopos,data_clump, window, infors,args.out)
