...
DataGWASComSubBonf<-DataGWASComSub[order(DataGWASComSubBonf[,ChroGW],DataGWASComSubBonf[,PosGW]),]

CmtCountSig=1
NbWind=1
GetBeginTab<-function(Nb)paste("\\begin{table}[ht]\n\\caption{ result for bonferroni correction table : ",Nb,"}\\centering\\begin{tabular}{c|c|c}",sep="")
LatexOut<-GetBeginTab(NbWind)
CmtSig=1
#for(CmtSig in 1:nrow(DataGWASComSubBonf)){
infosig<-DataGWASComSubBonf[CmtSig,]
Bloc<-DataHaplo[DataHaplo$CHR==infosig[,ChroGW]& DataHaplo$BP1<=infosig[,PosGW] & DataHaplo$BP2>=infosig[,PosGW],]
cat(nrow(Bloc))
cat(nrow(infosig))
cat('BP1',Bloc$BP1,'\n')
cat('BP2',Bloc$BP2,'\n')
cat('SizeWind',SizeWind,'\n')
#PlotWindLocusZoom(Chro,BeginBlock, EndBlock, WindSize,DataGWAS, ChroGW,PosGW, RsGW,PvalGW,DataGC,ChroGC, PosGC, DataClump, ListRs, DataGene,ChroGE, BeginGE, EndGE, NameGene, DataBlock){
