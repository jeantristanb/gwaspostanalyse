...
option_list = list(
  ...
  make_option("--res_clump", type="character",
...
filekniti='analyse_posgwascat_byclump.Rnw'
...
listhead=c('chro_gwas','bp_gwas','se_gwas','beta_gwas','rs_gwas','a1_gwas','a0_gwas','pval_gwas','af_gwas')
for(head in listhead){
if(!is.null(opt[[head]])){
if(!(opt[[head]] %in% names(DataGWAS))){
cat('not found header ', opt[[head]], 'in gwas file\n')
q(2)
}
}
}
...
DataResClumpI=read.table(opt[['res_clump']], header=T)
DirPWD=getwd()
...
cat('baa')
system('pdflatex analyse_posgwascat_byclump')
system('pdflatex analyse_posgwascat_byclump')
